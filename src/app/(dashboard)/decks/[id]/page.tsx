    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-ink-500 dark:text-ink-500">{deck.description}</p>
        </div>
      ...
      <Card>
        ...
        {cards.length === 0 ? (
          <p className="text-ink-500 dark:text-ink-500">No flashcards yet. Add some or generate with AI.</p>
        ) : (
          <ul className="divide-y divide-cream-100 dark:divide-ink-700">
            {cards.map((card) => (
              <li key={card.id} className="py-3">
                <p className="font-medium">{card.front}</p>
                <p className="text-sm text-ink-500 dark:text-ink-500">{card.back}</p>
              </li>
            ))}
          </ul>
        )}
